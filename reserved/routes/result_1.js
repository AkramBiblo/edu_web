        ...
        resultArray.forEach((data) => {
            ...
            function pushAttendance(TC, p, l, a, T) {
                if (T == 1) {
                    ...
                    TTermAttendance.push(TC)
                    TTermAttendance.push(p)
                    TTermAttendance.push(l)
                    TTermAttendance.push(a)
                } else if (T == 4) {
            ...
            con.connect(function(err) {
                 ...
                 let attdQ = `SELECT * FROM ${cls} WHERE month <= "3"`
                    ...
                    let attdQ_2 = `SELECT * FROM ${cls} WHERE month > "3" AND month <= "6"`
                ...
                let attdQ_3 = `SELECT * FROM ${cls} WHERE month > "6" AND month <= "9"`
                ...
                let attdQ_4 = `SELECT * FROM ${cls} WHERE month > "9" AND month <= "12"`
                con.query(attdQ_4, (err, result) => {
                    if (result.length <= 0) {
                    } else {
                        let T = 4;
                        let TC = result.length
            ...
            let sql = `SELECT * FROM first_r_play_nursery WHERE sid = "${sid}" AND cid = "${cid}"`;
                con.query(sql, function (err, result) {
                    if (result.length <= 0) {
                        ...
                        let third_term_allMarks = [];
                        let annual_term_allMarks = [];
                        let allData = {getFM, getSub, getPM, first_term_allMarks, second_term_allMarks, third_term_allMarks, annual_term_allMarks};
                        res.send(allData);
                    } 
                    else {
                        ...
                        let annual_term_allMarks = [];
                                ...
                                let allData = {getFM, getSub, getPM, first_term_allMarks, second_term_allMarks, third_term_allMarks, annual_term_allMarks, FTermAttendance};
                                ...
                                con.query(sql, function (err, result) {
                                    if (result.length <= 0) {
                                        ...
                                        let allData_2 = {getFM, getSub, getPM, first_term_allMarks, second_term_allMarks, third_term_allMarks, annual_term_allMarks, FTermAttendance, STermAttendance};
                                        
                                        let sql = `SELECT * FROM third_r_play_nursery WHERE sid = "${sid}"`
                                        con.query(sql, function (err, result) {
                                            if (result.length <= 0) {
                                                res.send(allData_2);
                                            } else {
                                                ...
                                                for (const data in result) {
                                                    if (Object.hasOwnProperty.call(result, data)) {
                                                        ...
                                                        new Promise((resolve, reject) => {
                                                            third_term_allMarks = [[arabic, islamiat, bangla, english, math, drawing, GK, handWriting], [calculatedGrade], [gradeObtained]];
                                                            resolve(third_term_allMarks)
                                                        })

                                                    }
                                                }

                                                let allData_3 = { getFM, getSub, getPM, first_term_allMarks, second_term_allMarks, third_term_allMarks, annual_term_allMarks, FTermAttendance, STermAttendance, TTermAttendance};
                                                ...
                                                con.query(sql, function (err, result) {
                                                    if (result.length <= 0) {
                                                        ...
                                                        let allData_4 = { getFM, getSub, getPM, first_term_allMarks, second_term_allMarks, third_term_allMarks, annual_term_allMarks, FTermAttendance, STermAttendance, TTermAttendance, ATermAttendance };
                                                        res.send(allData_4);
                                                    }
                                                })
                                            }
                                ...
                                for (const data in result) {
                                    if (Object.hasOwnProperty.call(result, data)) {
                                        ...
                                        for (let i = 0; i < g.length; i++) {
                                            ...
                                            if (devMark >= 80) {
                                                calculatedGrade = 'A+'
                                            } else if (devMark >= 70 && devMark < 80) {
                                                calculatedGrade = 'A'
                                            } else if (devMark >= 60 && devMark < 70) {
                                                calculatedGrade = 'A-'
                                            } else if (devMark >= 50 && devMark < 60) {
                                                calculatedGrade = 'B'
                                            } else if (devMark >= 33 && devMark < 50) {
                                                calculatedGrade = 'C'
                                            } else if (devMark < 33) {
                                                calculatedGrade = 'F'
                                            }  
...
result.get('/class/:cid', (req, res) => {
    ...
    if (cid == '99') {
        ...
        res.render('RUF_C6-C8', { title: 'Update result' })
    }
})
